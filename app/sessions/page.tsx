...
import { Radio, Calendar, Clock, Users, ArrowRight, Plus, Settings, Video, GraduationCap, Beaker, Atom } from "lucide-react";
...
export default async function SessionsPage() {
    ...
    const getStreamBadge = (stream: string) => {
        if (stream === "JEE") return (
            <span className="px-2 py-0.5 bg-orange-500/10 text-orange-400 border border-orange-500/20 rounded text-[10px] font-bold uppercase tracking-wider">JEE</span>
        );
        if (stream === "NEET") return (
            <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded text-[10px] font-bold uppercase tracking-wider">NEET</span>
        );
        return (
            <span className="px-2 py-0.5 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded text-[10px] font-bold uppercase tracking-wider">JEE + NEET</span>
        );
    };
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-6 pt-32 md:pt-40 pb-20 relative z-10">
                ...
                <div className="text-center max-w-3xl mx-auto mb-20 space-y-6 animate-fade-in-up">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-500/10 border border-purple-500/20 mb-2 shadow-[0_0_20px_rgba(168,85,247,0.15)]">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest text-purple-400">Live with Seniors</span>
                    </div>

                    <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-tight">
                        Talk to <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-cyan-400">IITians & AIIMSians</span> who cracked it.
                    </h1>

                    <p className="text-zinc-400 text-lg md:text-xl">
                        Join live sessions with seniors from IIT, AIIMS & top medical colleges. Ask questions, get strategies, and unlock the exact game-plan they used.
                    </p>
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-4 mt-8">
                            ...
                            <Button asChild variant="outline" className="bg-[#16133A]/60 border-white/10 text-white hover:bg-white/5 rounded-full px-8 py-6 backdrop-blur-xl">
                                <Link href="/admin/sessions">
                                    <Settings className="mr-2 w-5 h-5" /> Manage Sessions
                                </Link>
                            </Button>
                        </div>
                    )}
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
                        ...
                        >
                            <article className="bg-[#16133A]/60 backdrop-blur-xl border border-white/5 rounded-[32px] p-8 hover:border-purple-500/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(168,85,247,0.1)] flex flex-col h-full relative overflow-hidden">
                                ...
                                <div className="mb-3 relative z-10">
                                    {getStreamBadge(session.stream)}
                                </div>

                                <h3 className="text-xl font-bold text-white mb-2 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-purple-400 group-hover:to-cyan-400 transition-all duration-300 leading-tight relative z-10">
                                    {session.title}
                                </h3>

                                {/* Host info */}
                                <p className="text-purple-400 text-xs font-bold mb-3 relative z-10">{session.hostName} · {session.hostCollege}</p>

                                {session.description && (
                                    <p className="text-zinc-400/80 text-sm mb-8 line-clamp-2 leading-relaxed relative z-10">
                                        {session.description}
                                    </p>
                                )}

                                <div className="mt-auto space-y-4 mb-8 relative z-10">
                                ...
                                <div className="flex items-center justify-between border-t border-white/10 pt-6 relative z-10">
                                    <div className="flex items-center gap-2 text-sm font-bold text-purple-400 group-hover:text-purple-300 transition-colors">
                                        {session.status === "live" ? "Join Session Now" : "View Details"}
                    ...
                    {sessions.length === 0 && (
                        <div className="col-span-full py-32 text-center border border-dashed border-white/10 rounded-[32px] bg-[#16133A]/30 backdrop-blur-xl">
                            ...
                            <p className="text-indigo-300 font-bold text-lg mb-2">No upcoming sessions right now.</p>
                            <p className="text-zinc-500 text-sm">Check back soon — new IITian/AIIMSian sessions drop weekly!</p>
                        </div>
                    )}
                </div>
            </main>
